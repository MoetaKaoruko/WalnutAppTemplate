...
void Renderer::OnResize(uint32_t width, uint32_t height)
{
	...
	m_ImageHorizatalIterator.resize(width);
	m_ImageVerticalIterator.resize(height);

	for (int i = 0; i < width; i++)
		m_ImageHorizatalIterator[i] = i;
...
#define MT 1
	...
	[this](uint32_t y)
	{
		std::for_each(std::execution::par, m_ImageHorizatalIterator.begin(), m_ImageHorizatalIterator.end(),
		[this, y](uint32_t x)
		{
			...
			accumulateColor /= (float)m_FrameCount;
...
glm::vec4 Renderer::PerPixel(uint32_t x, uint32_t y)
{
	...
	for (int i = 0; i < bounces; i++)
	{
		...
		// ray.Direction = glm::reflect(ray.Direction, payload.WorldNormal + material.Roughness * Walnut::Random::Vec3());
